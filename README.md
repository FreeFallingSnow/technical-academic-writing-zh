...
大模型为明确遵循任务，常在生成过程中使用范围排除、风险提醒、审阅回应和规则例外。这些表述有助于控制生成，却可能被直接写入正式正文或最终回复。例如，研究对象和系统职责已经明确后，仍追加“本研究不涉及……”；完成修改后，又解释“规则并不禁止……”。这些内容没有向读者增加事实、方法或适用条件，会降低文档质量。

本 Skill 在起草、改写和终检阶段主动分离控制语言与交付语言。能够通过对象、职责、输入输出、用途和条件表达的边界直接写入相应叙述；上下文已经表达完整的重复声明予以删除；影响复现、解释、安全、责任或适用条件的独立限制继续保留。
...
规范直接作用于交付文本，不等待用户指出问题。模型先建立语义保护记录，再起草或修改正文，交付前检查重复、范围元话语、过程痕迹、结构和语义保持情况。用户只要求正文时，不附加“已优化”“修改如下”等说明。
...
1. 检索证据限定词、修改痕迹、范围元话语、表达辩护、否定词和总结词的组合；
...
8. 删除没有上下文依据的孤立排除项；
9. 保护直接引文和技术标识符；
10. 清理范围元话语和回复中的无关辩护，同时保留独立技术限制。
